...
def auto_zoom(ax, xi_na, field_na, xi_amr, field_amr, xi_exact, field_exact,
              has_na, has_amr, center, xi_max, half_width=0.06):
...
def main():
    os.makedirs("plot/result/result_2d_amr/diagonal", exist_ok=True)
    ...
    for tid in test_ids:
        ...
        has_na = False
        xi_na = rho_na = vn_na = p_na = e_na = None
        if os.path.exists(rd):
            try:
                pf = find_final_plotfile(rd)
                print(f"  Reading no-AMR: {pf}")
                x, y, data, _, _ = read_amrex_plotfile(pf)
                rho_2d, vn_2d, p_2d, e_2d = compute_primitives(data, GAMMA)
                xi_na, rho_na = extract_diag_slice(x, y, rho_2d)
                _, vn_na = extract_diag_slice(x, y, vn_2d)
                _, p_na = extract_diag_slice(x, y, p_2d)
                _, e_na = extract_diag_slice(x, y, e_2d)
                if tid == 2:
                    e_isentropic_na = 1.0 * rho_na ** (GAMMA - 1.0)
                    rho_lo, rho_hi = 0.02, 0.15
                    alpha_na = np.clip((rho_na - rho_lo) / (rho_hi - rho_lo), 0.0, 1.0)
                    e_na = alpha_na * e_na + (1.0 - alpha_na) * e_isentropic_na
                has_na = True
            except Exception as ex:
                print(f"  Error: {ex}")

        # AMR (composite)
        rd2 = os.path.join("output_2d", "diagonal_amr", T['dir_amr'])
        has_amr = False
        xi_amr = rho_amr = vn_amr = p_amr = e_amr = None
        if os.path.exists(rd2):
            try:
                pf2 = find_final_plotfile(rd2)
                print(f"  Reading AMR (composite): {pf2}")
                xi_amr, rho_amr, vn_amr, p_amr, e_amr = extract_diag_slice_composite(pf2, GAMMA)
                if tid == 2:
                    rho_lo, rho_hi = 0.02, 0.15
                    e_isentropic_amr = 1.0 * rho_amr ** (GAMMA - 1.0)
                    alpha_amr = np.clip((rho_amr - rho_lo) / (rho_hi - rho_lo), 0.0, 1.0)
                    e_amr = alpha_amr * e_amr + (1.0 - alpha_amr) * e_isentropic_amr
                has_amr = True
            except Exception as ex:
                print(f"  Error: {ex}"); import traceback; traceback.print_exc()

        if not has_na and not has_amr:
            print("  No data"); continue

        # Find zoom center
        if has_na and has_amr:
            zoom_center = find_max_diff_location(xi_na, rho_na, xi_amr, rho_amr)
            print(f"  Max difference at xi = {zoom_center:.4f}")
        else:
            zoom_center = T['x0'] * np.sqrt(2.0)

            fig, axes = plt.subplots(2, 2, figsize=(14, 10))

        na_f = [rho_na, vn_na, p_na, e_na]
        amr_f = [rho_amr, vn_amr, p_amr, e_amr]
        ex_f = [rho_ex, u_ex, p_ex, e_ex]
        titles = ['Density', 'Normal Velocity', 'Pressure', 'Internal Energy']

        for idx, ax in enumerate(axes.flat):
            if has_na:
                ax.plot(xi_na, na_f[idx], 'go', markersize=2, alpha=0.4, label='No AMR (400×400)')
            if has_amr:
                ax.plot(xi_amr, amr_f[idx], 'r.', markersize=1.0, alpha=0.6, label='AMR (400×400)')
            ...
            if has_na and has_amr:
                auto_zoom(ax, xi_na, na_f[idx], xi_amr, amr_f[idx],
                         xi_exact, ex_f[idx], has_na, has_amr, zoom_center, xi_max)

        plt.tight_layout()
        out = f"plot/result/result_2d_amr/diagonal/2d_diag_amr_{T['dir']}.png"
